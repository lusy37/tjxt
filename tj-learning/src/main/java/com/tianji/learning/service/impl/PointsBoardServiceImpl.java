...
public class PointsBoardServiceImpl extends ServiceImpl<PointsBoardMapper, PointsBoard> implements IPointsBoardService {
    ...
    public void createPointsBoardTableBySeason(Integer season) {
        this.getBaseMapper().createPointsBoardTable(POINTS_BOARD_KEY_PREFIX + season);
    }
    ...
    private List<PointsBoard> queryHistoryBoardList(PointsBoardQuery query) {
        // TODO
        // 封装表名
        String key = POINTS_BOARD_KEY_PREFIX + query.getSeason();
        TableInfoContext.setInfo(key);
        // 分页查询历史数据
        Page<PointsBoard> page = lambdaQuery()
        ...
        for (PointsBoard record : records) {
            record.setRank(Math.toIntExact(record.getId()));
    ...
    private PointsBoard queryMyHistoryBoard(Long season) {
        // TODO
        // 封装表名
        String key = POINTS_BOARD_KEY_PREFIX + season;
        TableInfoContext.setInfo(key);
        PointsBoard myBoard = lambdaQuery()
                .select(PointsBoard::getId, PointsBoard::getUserId, PointsBoard::getPoints)
        ...
        myBoard.setRank(Math.toIntExact(myBoard.getId()));
        return myBoard;
    }
